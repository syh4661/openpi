...
# OpenPI imports
try:
    from openpi.policies import openarm_runtime_contract as _runtime_contract
except ImportError as e:
...
class InferenceConfig:
    ...
    joint_names: list = field(
        default_factory=lambda: [
            ...
            "openarm_left_finger_joint2",  # Left gripper
            ...
            "openarm_right_finger_joint2",  # Right gripper
        ]
    )
...
class OpenArmInferenceNode(Node):
    ...
    def get_observation(self) -> dict | None:
        ...
        with self._obs_lock:
            ...
            # Convert ROS HWC images into canonical OpenArm CHW runtime layout.
            images = {}
            for cam_name, img in self._images.items():
                ...
                images[cam_name] = np.transpose(img, (2, 0, 1)).copy()
...
def run_inference(config: InferenceConfig, task: str):
    ...
    try:
        runtime_policy = _runtime_contract.create_runtime_policy(
        ...
        logger.info(f"Contract: {runtime_policy.metadata['version']}")
    ...
    try:
        while frame_count < config.max_steps:
            ...
            try:
                result = runtime_policy.infer(obs)
                action_np = np.asarray(result["actions"][0], dtype=np.float32)
...
def main():
    ...
    parser.add_argument("--config", type=str, default="pi05_openarm", help="Training config name")
    parser.add_argument("--max-steps", type=int, default=10000, help="Maximum number of inference steps")
    parser.add_argument("--fps", type=float, default=10.0, help="Inference frequency (frames per second)")
